...
class LapHistoryDataParser extends F1Parser {
  constructor() {
    super();
    this.uint32le('m_lapTimeInMS').uint16le('m_sector1TimeInMS').uint8('m_sector1TimeMinutes').uint16le('m_sector2TimeInMS').uint8('m_sector2TimeMinutes').uint16le('m_sector3TimeInMS').uint8('m_sector3TimeMinutes').uint8('m_lapValidBitFlags');
  }
}
